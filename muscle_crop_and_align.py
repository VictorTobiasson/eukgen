...
def realign_cluster_fasta(fasta, threads, max_sequences=250, filter_entropy=0, 
                          gap_fraction=0.5, muscle_reps=10, predelete_outliers = False, 
                          save_intermediate_files=False, muscle_timeout=7200):
    ...
    if size > max_sequences:
        ...
        fasta = fasta_reduce_size(fasta, threads, max_sequences, crop_entropy, 
                                  save_intermediate_files=save_intermediate_files, 
                                  predelete_outliers = predelete_outliers)
...
parser.add_argument('--gap_fraction', type=float, required=False, nargs='?', const=1, default=1, help='fraction of total gaps which to allow, ex 0.2 allows maximum 20 percent gaps per column')
...
parser.add_argument('--predelete_outliers', action=argparse.BooleanOptionalAction, help='Crop 10th percentile long leaf outliers and 5th percentile long leaf clades before cropping to size')
...
if __name__ == '__main__':
    ...
    realign_cluster_fasta(fasta=args.fasta,
                          ...
                          predelete_outliers = args.predelete_outliers,
