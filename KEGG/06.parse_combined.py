...
uid_taxids = "/data/luojaa/uid2taxids.combined.tsv"
log_path = "/data/luojaa/log/parse_fastas.combined.log"
skip = True
    ...
    for record in SeqIO.parse(handle, 'fasta'):
        ...
        try:
            if isEuk(taxid):
                ...
                outfasta=f"/data/luojaa/kog_fastas/{kog}.fasta"
                with open(outfasta, "a") as cluster:
                    print(">" + uid, file=cluster)
                    print(sequence, file=cluster)
                euk_count += 1
        except:
            ete3logfile = "/data/luojaa/log/ete3log.fasta"
